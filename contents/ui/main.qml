...
PlasmoidItem {
    ...
    Plasma5Support.DataSource {
        ...
        function run(command) { execute.connectSource(command) }
    }
    fullRepresentation: ColumnLayout {
        ...
        function runRyzenAdjCommand(args) {
            ...
            let command = `bash -c "echo 'Running command...'; sudo ${path} ${safeArgs}"`;
            execute.run(command);
        }
        ...
        function runCommand3() {
            ...
            let command = `bash -c "sudo ${path} ${value}"`;
            execute.run(command);
            updateOutput(`Applied Preset 1: ${value}`);
        }
